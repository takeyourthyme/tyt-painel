...
interface CalendarProps extends AriaCalendarProps<DateValue> {
    ...
    /** Label for the quick "today" control (default: `Today`). */
    todayLabel?: string;
    /** Weekday header width: one letter (e.g. Figma) or two (default). */
    weekdayLetterLen?: 1 | 2;
}

export const Calendar = ({ highlightedDates, todayLabel, weekdayLetterLen = 2, className, ...props }: CalendarProps) => {
        ...
        <ContextWrapper>
            <AriaCalendar {...props} className={(state) => cx("flex flex-col gap-3", typeof className === "function" ? className(state) : className)}>
                ...
                <div className="flex gap-3">
                    ...
                    <PresetButton value={today(getLocalTimeZone())}>{todayLabel ?? "Today"}</PresetButton>
                </div>
