...
import { Download02, Plus } from "@untitledui/icons";
...
import { postPratoFromFields } from "@/lib/tyt-api/pratos";
...
export default function DishNewPage() {
    ...
    const loadCatalogs = useCallback(async () => {
        ...
        setError(null);
        try {
            const [catsRes, cuisineRes, themesRes, mainRes, prefRes, ingredientsRes] = await Promise.all([
            ...
            const [catsJson, cuisineJson, themesJson, mainJson, prefJson, ingredientsJson] = await Promise.all([
    ...
    const foto1Preview = form.foto1File ? URL.createObjectURL(form.foto1File) : null;
    const foto2Preview = form.foto2File ? URL.createObjectURL(form.foto2File) : null;
            ...
            <div className="mx-auto flex w-full max-w-[1372px] flex-col gap-6">
                <header className="flex flex-col gap-3">
                    <div className="flex items-center gap-2 text-sm text-tertiary">
                        ...
                        <span>/</span>
                        <span className="text-primary">Novo prato</span>
                    </div>
                    ...
                    <>
                            ...
                            <article className="rounded-xl bg-primary shadow-xs ring-1 ring-secondary ring-inset">
                                ...
                                <div className="flex flex-col gap-4 px-5 py-5">
                                    <Input
                                        ...
                                        placeholder="Ex: Salada Verde com Molho de Mostarda e Mel"
                                    />
                                    <TextArea
                                        label="Descrição"
                                ...
                                <div className="flex flex-col gap-4 px-5 py-5">
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        <div className="flex flex-col gap-2">
                                            ...
                                            {form.foto1File ? (
                                                <Button color="link-destructive" size="sm" onClick={() => setForm((p) => ({ ...p, foto1File: null }))}>
                                                    Remover foto 1
                                                </Button>
                                            ) : null}
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            {form.foto2File ? (
                                                <Button color="link-destructive" size="sm" onClick={() => setForm((p) => ({ ...p, foto2File: null }))}>
                                                    Remover foto 2
                                                </Button>
                                            ) : null}
                                        </div>
                            ...
                            <div className="grid gap-6 px-5 py-5 lg:grid-cols-2">
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    {foodPreferenceSelection.size ? (
                                        <TagGroup label="Preferência culinária selecionada" size="md">
                                            <TagList className="flex flex-wrap gap-2">
                        ...
                        <section className="grid gap-6 lg:grid-cols-2">
                            <article className="rounded-xl bg-primary shadow-xs ring-1 ring-secondary ring-inset">
                                ...
                                <div className="flex flex-col gap-2 px-5 py-5">
                                    ...
                                    />
                                    {form.fichaTecnicaFile ? <p className="text-sm text-tertiary">{form.fichaTecnicaFile.name}</p> : null}
                                    {form.fichaTecnicaFile ? (
                                        <Button color="link-destructive" size="sm" onClick={() => setForm((p) => ({ ...p, fichaTecnicaFile: null }))}>
                            ...
                            <article className="rounded-xl bg-primary shadow-xs ring-1 ring-secondary ring-inset">
                                ...
                                <div className="flex flex-col gap-2 px-5 py-5">
                                    ...
                                    />
                                    {form.receitaFile ? <p className="text-sm text-tertiary">{form.receitaFile.name}</p> : null}
                                    {form.receitaFile ? (
                                        <Button color="link-destructive" size="sm" onClick={() => setForm((p) => ({ ...p, receitaFile: null }))}>
                                            Remover receita
                                        </Button>
                                    ) : null}
                                </div>
                        ...
                        <footer className="flex flex-col-reverse items-center justify-between gap-3 border-t border-secondary pt-5 sm:flex-row">
                            ...
                            <div className="flex w-full gap-3 sm:w-auto">
                                <Button color="secondary" size="md" className="flex-1 sm:flex-none" onClick={() => router.push("/cardapio")}>
                                    Cancelar
                                </Button>
                                <Button
                                    color="primary"
                                    ...
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            const createdId = getNumberValue(record, ["id"]);
